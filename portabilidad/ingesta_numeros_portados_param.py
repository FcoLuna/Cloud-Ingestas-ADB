...
try:
    df_tmp1 = df.select(
        df['value'], 
        F.split(df['value'], ',').alias('split_column')
    )
    display(df_tmp1)
except Exception as e:
    # Capturar cualquier excepción y mostrar un mensaje de error
...
try:
    ...
    .withColumnRenamed("col_9", "modalidad") \
    .withColumnRenamed("col_10", "rut_del_solicitante") \
    .withColumnRenamed("col_11", "id_portabilidad") \
    .withColumnRenamed("col_12", "receptor") \
    .withColumnRenamed("col_13", "donante") \
    .withColumnRenamed("col_14", "fecha_de_portacion") 
...
try:
        ...
        "fecha_de_portacion", 
        concat(
        substring(col("fecha_de_portacion"), 7, 4),
        lit("-"),
        substring(col("fecha_de_portacion"), 0, 2),
        lit("-"),
        substring(col("fecha_de_portacion"), 4, 2),
        lit(" "),
        substring(col("fecha_de_portacion"), 12, 2),
        lit(":"),
        substring(col("fecha_de_portacion"), 15, 2),
        lit(":"),
        substring(col("fecha_de_portacion"), 18, 2)
        )
        ) \
        .withColumn(
...
# MAGIC %md
# MAGIC Borrando Duplicados Detalles Solicitudes Números Portados

# COMMAND ----------

#df_duplicados = spark.sql("SELECT * FROM bi_ingestas.raw_interacciones.detalle_numeros_portados")
try:
    df_duplicados = spark.sql(f"""SELECT id_operador, hora_envio_solicitud, nt,tipo_de_servicio_donante, tipo_de_servicio_receptor, region, comuna, localidad, 
    modalidad, rut_del_solicitante, id_portabilidad, receptor, 
    donante, fecha_de_portacion FROM {full_table_name_append}""")
except Exception as e:
    # Capturar cualquier excepción y mostrar un mensaje de error
    status = "[ERROR]"
    desc_status = f"[ERROR] : {e}"
...
try:
    ...
    df_sin_duplicados = df_sin_duplicados_group.select(
        ...
        "tipo_de_servicio_receptor", "region", "comuna", "localidad", 
        "modalidad", "rut_del_solicitante", "id_portabilidad", "receptor", 
        "donante", "fecha_de_portacion"
    )
