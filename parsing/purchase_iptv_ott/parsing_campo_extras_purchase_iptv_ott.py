...
'''ruta_stage      = "abfss://ingestas@stbigdatadev02.dfs.core.windows.net/data/ventas/ventas_sva/gvp/purchases/stage/temp_ott"
nombre_archivo  = "gvp_purchase_ott"
delimitador     = ";"
encabezado      = "true"
encoding        = "UTF-8"
quote           = "\""
origen          = "ott"
ruta_landing    ="abfss://ingestas@stbigdatadev02.dfs.core.windows.net/data/ventas/ventas_sva/gvp/purchases/landing"
'''
...
for archivo in directorio:
    ...
    print(f"fecha:{fecha}  archivos :{archivo.name}")
...
schema = StructType([
    ...
    StructField("producername",StringType(),True),
    StructField("distributorname",StringType(),True),
    StructField("commercializationtype",StringType(),True),
    StructField("contentcategoryid",StringType(),True),
    StructField("tvodtypeid",StringType(),True),
    StructField("quality",StringType(),True),
    StructField("price",StringType(),True),
    StructField("useremail",StringType(),True),
    StructField("useruniqueid",StringType(),True),
    StructField("firstname",StringType(),True),
    StructField("lastname",StringType(),True),
    StructField("servicetype",StringType(),True),
    StructField("devicetypename",StringType(),True),
    StructField("paymenttype",StringType(),True),
    StructField("webpaytransactionid",StringType(),True),
    StructField("idwebpay",StringType(),True),
    StructField("authwebpaycode",StringType(),True),
    StructField("purchasestatus",StringType(),True),
    StructField("transactionhash",StringType(),True),
    StructField("syncmessage",StringType(),True),
    StructField("deviceid",StringType(),True),
    StructField("billingid",StringType(),True),
    StructField("price_full",StringType(),True),
...
for fecha, archivos in archivos_por_fecha.items():
    ...
    ''' df_temp = spark.read.csv(archivos[0].path,header=encabezado,encoding=encoding,sep=delimitador,quote=quote,schema=schema).withColumn(
        "filename_spark",
        concat(lit("Chile"), lit(origen), lit("_LogLogin_"), lit(filename_spark))
    ).withColumn(
        "ts",
        concat(regexp_replace(split(split(lit(archivos[0].name), ".")[0], "_")[2], "-", ""),
               split(split(split(lit(archivos[0].name), ".")[0], "_")[3], "-")[0])
    )'''
    salida1 = split(split(lit(archivo.name), ".csv")[0],"_")[2]
    #salida2 = split(split(lit(archivo.name), ".csv")[0],"_")[0], "_")[3]
    salida_split = concat(split(split(lit(archivo.name), ".")[0], "_")[3], lit("_"),regexp_replace(split(lit(archivo.name), "_")[2], "-", ""))

    print(f"salida split1: {salida1}")
    print(f"salida split: {salida_split}")
    ...
    dataframes = [
        ...
        .withColumn("filename_spark",
                    concat(lit("Chile"), lit(origen), lit("_Purchases_"), split(split(lit(archivo.name),".csv")[0],"_")[3], lit("_") , regexp_replace(split(lit(archivo.name), "_")[2],"-",""), lit(".csv"))
        )
    ...
    print(f"largo dataframes :{len(dataframes)}")
