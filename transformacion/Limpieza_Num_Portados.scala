...
WHERE
id_operador like '%DETALLES SOLICITUD DE PORTABILIDAD%' 
OR id_operador like '%ID Operador%'
OR hora_envio_solicitud like '%Hora Envio Solicitud%'
OR nt like '%NT%'
OR tipo_de_servicio_donante like '%Tipo de Servicio (Donante)%'
OR tipo_de_servicio_receptor like '%Tipo de Servicio (Receptor)%'
OR comuna like '%Comuna%'
OR localidad like '%Localidad%'
OR modalidad like '%Modalidad%'
OR rut_del_solicitante like '%RUT del Solicitante%'
