...
dbutils.widgets.text("ruta_origen","/data/trafico/trafico_detalle/smartcare/tdr_ims_ip_message_sip_test/landing")
dbutils.widgets.text("path_salida","/data/trafico/trafico_detalle/smartcare/tdr_ims_ip_message_sip/raw")
dbutils.widgets.text("catalogo","bidesarrollo")
dbutils.widgets.text("tabla_salida","raw_trafico.smartcare_tdr_ims_ip_message_sip")*/
...
val columnas = Seq("STARTTIME","MILLISEC","ENDTIME","SERVICE_TYPE","SERVICE_STATUS","ACCESS_NE_TYPE","ACCESS_NE_IP","ACCESS_NE_PORT","ACCESS_NE_ID","P_CSCF_IP","P_CSCF_PORT","P_CSCF_ID","S_CSCF_IP","S_CSCF_PORT","S_CSCF_ID","I_CSCF_IP","I_CSCF_PORT","I_CSCF_ID","AS_IP","AS_PORT","AS_ID","SLF_IP","SLF_PORT","SLF_ID","HSS_IP","HSS_Port","HSS_ID","IP_SM_GW_IP","IP_SM_GW_PORT","IP_SM_GW_ID","IP_SM_GW_ADDR","HLR_ADDR","SMSC_ADDR","MESSAGE_LENGTH","IMPI_TEL_URI","IMPI_SIP_URI","IMPU_TEL_URI","IMPU_SIP_URI","IMPU_TYPE","IMEI","TERM_UNTRUST_IP_ADDR","TERM_UNTRUST_PORT","TERM_TRUST_IP_ADDR","TERM_TRUST_PORT","DEVICE_TYPE","ACCESS_TYPE","ACCESS_INFO","VISIT_DOMAIN","HOME_DOMAIN","CALLING_PARTY_ADDRESS","CALLING_PARTY_TYPE","CALLING_PARTY_SIP_URI","CALLED_PARTY_ADDRESS","CALLED_PARTY_TYPE","CALLED_PARTY_SIP_URI","RESPONSE_CODE","RP_ERROR","TP_ERROR","FINISH_WARNING","FINISH_REASON","FIRFAILTIME","FIRST_FAIL_NE_TYPE","FIRST_FAIL_NE_IP","FIRST_FAIL_NE_NI","FIRST_FAIL_NE_PC","Layer1ID","Layer2ID","Layer3ID","Layer4ID","Layer5ID","Layer6ID","RESERVED1","RESERVED2","RESERVED3","RESERVED4","RESERVED5","RESERVED6","RESERVED7","RESERVED8","MCC","MNC","HOMEMCC","HOMEMNC","RAT","MME_ID","MME_SIG_IP","SGW_ID","SGW_IP","LTE_RAN_NE_ID","LTE_RAN_NE_IP","MME_S1APID","eNodeB_S1APID","_corrupt_record")
...
val df_origen = spark.readStream
  ...
  .option("cloudFiles.schemaLocation", s"${path_adls_smartcare}/schemas/tdr_ims_ip_message_sip") // Ruta para el esquema
...
val query = df.writeStream
  ...
  .option("checkpointLocation", s"${path_adls_smartcare}/checkpoints/tdr_ims_ip_message_sip") // Ruta de checkpoints
...
while (query.isActive) {
  ...
  if (lastProgress != null) {
    ...
    println(s"📌 Último Checkpoint: ${s"${path_adls_smartcare}/checkpoints/tdr_ims_ip_message_sip"}")
