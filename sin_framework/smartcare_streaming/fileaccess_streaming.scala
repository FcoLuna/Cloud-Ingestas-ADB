...
val fileaccess = spark.readStream
  .format("cloudFiles")
  ...
  .option("cloudFiles.schemaLocation", s"${path_adls_smartcare}/schemas/fileaccess") // Ruta para el esquema
...
val query = df.writeStream
  ...
  .option("checkpointLocation", s"${path_adls_smartcare}/checkpoints/fileaccess") // Ruta de checkpoints
...
while (query.isActive) {
  ...
  if (lastProgress != null) {
    ...
    println(s"📌 Último Checkpoint: ${s"${path_adls_smartcare}/checkpoints/fileaccess"}")
