...
val stagePath = s"$adls_path/$dir_adls_rel/stage_3G"
...
filesByFamily.foreach { case (family, filePaths) =>
  ...
  // Leer y combinar los archivos de la misma familia
  ...
  // Escribir en la tabla Delta si hay datos válidos
...
// COMMAND ----------
  ...
  val delete3G = s"$adls_path/$dir_adls_rel/stage_3G"
  println(delete3G)
  dbutils.fs.rm(delete3G, true)
