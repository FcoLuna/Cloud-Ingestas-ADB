...
// MAGIC %run ../ingesta_bd_csv_sink/funciones_genericas_csv
...
if(status_ejecucion == 0){
    try {
        ...
        val partitions = df_sal1.select(columns.map(col): _*).distinct.collect
        // Ejecutar la optimización para cada partición
        val deltaTable = DeltaTable.forPath(spark, path_data)
        partitions.foreach { row =>
        val filterExpr = columns.zipWithIndex.map { case (colName, idx) =>
            s"$colName = '${row.getString(idx)}'"
          }.mkString(" AND ")
          
          println("Inicia Optimize particion: " + filterExpr)
          deltaTable.optimize().where(filterExpr).executeCompaction()
        }
