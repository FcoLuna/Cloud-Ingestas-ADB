...
var numPartitions, dif_row_count, repartition = 0
...
if(status_ejecucion == 0){
    try {
      ...
      // cantidad particiones df
      repartition = dbutils.widgets.get("repartition").toInt
...
if(status_ejecucion == 0){
    try {
      ...
      df = df.repartition(repartition).withColumn("bigdata_close_date",lit(null)).withColumn("bigdata_ctrl_id", lit(bigdata_ctrl_id) cast "long")
      
      //se eliminan valores nulos y duplicados
      println("SE ELIMINAN DUPLICADOS y NULOS")
      df_sal1 = df.dropDuplicates
      df_sal1 = df_sal1.na.drop("all")
      df_sal1 = df_sal1.persist(StorageLevel.MEMORY_AND_DISK) 
      ...
      println("Valida campos partition_date_column y auxiliar_partition_value") 
      if (partition_date_column == "" && auxiliar_partition_value != "") {
          df_sal1 = df_sal1.withColumn("bigdata_close_date", lit(auxiliar_partition_value) cast "date")
          ...
          df_sal1 = df_sal1.withColumn("bigdata_close_date", lit(bigdata_close_date) cast "date")
          partition_date_column = "bigdata_close_date"
          formato_partition_column = "yyyy-MM-dd"
        }

      println("Valida partitioned by")  
      if (create_table_query_hql.toLowerCase().contains("partitioned by")) {
          ...
          println("Crea columna bigdata_close_date")
          df_sal1 = df_sal1.withColumn("bigdata_close_date", to_date(col(partition_date_column), formato_partition_column_out))
            
          //SE OBTIENE EL NOMBRE DE LA COLUMNA O COLUMNAS DE PARTICION DESDE EL HQL. ej: (partition_date,year,month,day)
          println("Obtiene los nombres de las columnas de particion")
          ...
          println("Separa las columnas de particion")
          ...
          val selectExprs = df_sal1.columns.map(col) ++ (0 until columns.size  map (i => $"tmp".getItem(i).as(columns(i))))
          
          //SE APLICA EL SELECT CREADO AL DATAFRAME
          df_sal1 = df_sal1.withColumn("tmp", split($"bigdata_close_date", "-")).select(selectExprs:_*)
          df_sal1 = df_sal1.persist(StorageLevel.MEMORY_AND_DISK)
      }
    ...
    try {
        if (create_table_query_hql.toLowerCase().contains("partitioned by")) {
          ...
          if (loadType == "incremental"){
            println("Si es incremental y tiene partitioned by, Hacer Merge")
            salida_merge = mergeData(path_data, merge_string, df_sal1, true, columns)
          ...
          if (loadType == "incremental"){
            ...
            salida_merge = mergeData(path_data, merge_string, df_sal1, false, columns)
        ...
        //VALIDA EJECUCION MERGE
          if (!salida_merge) {
            println("MERGE falló.")
            status_ejecucion = 1
            } else {
              println("MERGE se ejecutó correctamente.")
              status_ejecucion = 0 
              }
