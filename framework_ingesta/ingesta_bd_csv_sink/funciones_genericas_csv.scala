...
// MAGIC %md
// MAGIC ### ELIMINA DATOS DIARIOS

// COMMAND ----------

//función que elimina los datos de ejecución diaria antes de guardarlos, esto evitara duplicidad por el campo bigdata_ctrl_id en caso de reprocesos en un mismo día
def elimina_datos_periodicidad(tablauc: String, fecha: String, periodicidad: String): Unit = {

     if (periodicidad == "d") {
      spark.sql(s"delete from ${tablauc} where bigdata_close_date = '${fecha}'")
      println(s"Registro eliminado de la tabla ${tablauc} para la fecha ${fecha}")
  } else {
    println(s"No se pudo eliminar la tabla ${tablauc}")
    }
      }
  ...
  try{
   ...
   case e: Exception =>
    ...
    if (!tableExists && (destination_path.contains("raw") || destination_path.contains("noraw") || destination_path.contains("conformado"))){
      if (partition) {
        println(s" La tabla no existe, crea tabla con particion ")
        ...
        println(" La tabla no existe, crea tabla sin particion ")
        df.write.format("delta")
                .mode("overwrite")
                .option("overwriteSchema", "true")
                .save(destination_path)
