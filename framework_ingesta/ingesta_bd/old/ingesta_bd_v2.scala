...
// REPLACE LOCATION MANAGED AND TABLE CATALOG.SCHEMA.TABLE
...
if (subida_datos.toLowerCase().contains("partitioned by")) {
  ...
  if (loadType == "i"){
    ...
    mergeData(path_data, merge_string, df)
...
if (subida_datos.toLowerCase().contains("partitioned by")) {
  ...
  println("tabla cuenta con la particion de la columna " + partition_name)
