...
// MAGIC %sql
// MAGIC DROP TABLE devtmp.raw_finanzas.ingreso3
...
// ESTO SÓLO ES PARA NOSOTROS QUE NO TENEMOS PERMISO DE CREAR ESQUEMAS
val dir_adls_schema  = dbutils.widgets.get("dir_adls_schema") 
//PARÁMETRO SCHEMA
...
// //CREA SCHEMA EN CASO DE NO EXISTIR
var nombre_catalogo_schema = nomb_proc.split("\\.")(0) + "." + nomb_proc.split("\\.")(1)
try {
println(s"CREATE SCHEMA IF NOT EXISTS $nombre_catalogo_schema MANAGED LOCATION '$dir_adls_schema'")
spark.sql(s"CREATE SCHEMA IF NOT EXISTS $nombre_catalogo_schema MANAGED LOCATION '$dir_adls_schema'")
} catch {
  case e: Exception =>
    {
      println("[ERROR] " + e)
...
//REVISAR
//CONSIDERAR QUE SI LA RUTA ESTÁ MALA, DEBIESE CAERSE EL PROCESO

//en ocaciones en el origen cambian las columnas, por lo tanto deberian extraer solo las deseadas.
//validaciones cuando se encuentre un schema distinto

// COMMAND ----------

val subida_datos =  openFile(dir_adls + "/" + nombre_archivo + ".hql")

// display(subida_datos)
