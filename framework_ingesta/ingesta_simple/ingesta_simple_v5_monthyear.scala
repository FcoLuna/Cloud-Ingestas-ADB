          ...
          if (create_table_query_hql.toLowerCase().contains("partitioned by")) {
            ...
            if (exists_file(dir_adls + "/transformation/transformation.json")) {
                ...
                df2 = spark.read.option("delimiter", delimitador).option("header", encabezado).
                  option("mode", "FAILFAST").option("columnNameOfCorruptRecord", "_corrupt_record").schema(schema_data).option("DateFormat", dateFormatFile).option("timestampFormat", timestampFormat).csv(path_tmp_transform + "*") 
                ...
                df2 = spark.read.option("delimiter", delimitador).option("header", encabezado).
                  option("mode", "FAILFAST").option("columnNameOfCorruptRecord", "_corrupt_record").schema(schema_data).option("DateFormat", dateFormatFile).option("timestampFormat", timestampFormat).csv(path_processing + filename)
            ...
            //CREA UN FORMATO DE FECHA PARA EL CAMPO YEARMONTH
            val format_yyyyMM_id = new SimpleDateFormat("yyyyMM")
            yearmonth = format_yyyyMM_id.format(current_id)

            // SE AGREGA LA COLUMNA YEARMONTH, BIGDATA_CLOSE_DATE Y BIGDATA_CTRL_ID AL DATAFRAME
            df3 = df3.withColumn("yearmonth", lit(yearmonth)).
            withColumn("bigdata_close_date", to_date(lit(partition_value), formato_salida)).
            withColumn("bigdata_ctrl_id", lit(bigdata_ctrl_id) cast "long")

                        df3.show(false)

            println("[INFO] SE AGREGA LAS COLUMNAS  YEARMONTH, BIGDATA_CLOSE_DATE Y BIGDATA_CTRL_ID AL DATAFRAME OK")
            ...
            if (create_table_query_hql.toLowerCase().contains("partitioned by")) {
                ...
                if ( cant_repartition == 0 ){
                    ...
                    df_sal1.repartition(cant_repartition).write.partitionBy(columns: _*).format("delta").option("partitionOverwriteMode", "dynamic").option("mode", "FAILFAST").option("columnNameOfCorruptRecord", "_corrupt_record").mode(SaveMode.Overwrite).save(path_data)
                    ...
                    df_sal1.repartition(numPartitions).
                    write.partitionBy(columns: _*).
                    format("delta").
                    option("partitionOverwriteMode", "dynamic").
                    mode(SaveMode.Overwrite).save(path_data)
                }
