          ...
          if (create_table_query_hql.toLowerCase().contains("partitioned by")) {
            ...
            if (create_table_query_hql.toLowerCase().contains("partitioned by")) {
                ...
                if ( cant_repartition == 0 ){
                    ...
                    df_sal1.repartition(cant_repartition).write.partitionBy(columns: _*).format("delta").mode(SaveMode.Append).save(path_data)                     
                    ...
                    //df_sal1.repartition(numPartitions).write.partitionBy(columns: _*).format("delta").option("partitionOverwriteMode", "dynamic").mode(SaveMode.Overwrite).save(path_data)
                     df_sal1.repartition(numPartitions).write.partitionBy(columns: _*).format("delta").mode(SaveMode.Append).save(path_data)                    
                }
                ...
                if ( cant_repartition == 0 ){
                    ...
                    df_sal1.write.format("delta").mode(SaveMode.Append).save(path_data)                   
                } else if ( cant_repartition >= 1 ){
                    //df_sal1.repartition(cant_repartition).write.format("delta").mode(SaveMode.Overwrite).save(path_data)
                    df_sal1.repartition(cant_repartition).write.format("delta").mode(SaveMode.Append).save(path_data)                    
                } else if ( cant_repartition == -1 ){
                    println("[INFO] SE ESCRIBE EL DATAFRAME A UN DIRECTORIO TEMPORAL")
                    ...
                    df_sal1.repartition(numPartitions).write.format("delta").mode(SaveMode.Append).save(path_data)
                }
