...
/* %sql
drop table bidesarrollo.raw_producto_asignado.parque_ott_test_ingesta_simple */

// COMMAND ----------

/* dbutils.fs.cp(
  "abfss://bigdatadev@stbigdatadev02.dfs.core.windows.net/data/producto_asignado/productos_sva/gestor_ott/parque_ott_test_ingesta_simple/aux_copy_landing", 
  "abfss://bigdatadev@stbigdatadev02.dfs.core.windows.net/data/producto_asignado/productos_sva/gestor_ott/parque_ott_test_ingesta_simple/landing",
  true
)  */
...
var dataframe1, df1, df2, df3, df_sal1, df_sal2 : DataFrame = null
...
val start_time_adb_date = formatter.format(start_time)
val start_time_adb = start_time_adb_date.toString

val end_time = new Date().getTime
val end_time_adb_date = formatter.format(end_time)
val end_time_adb = end_time_adb_date.toString
val totaltime_adb_date = (end_time - start_time).toFloat / 1000
val totaltime_adb = totaltime_adb_date.toString
 */
...
display(spark.sql(s"select * from bidesarrollo.control.control_ingestas where process_name like '$process_name' order by starttime_spark desc"))
