...
if(status_ejecucion == 0){
    try {
        ...
        dir_adls = "abfss://ingestas@stbigdatadev02.dfs.core.windows.net/data/interacciones/ordenes/sbe/compra_equipo_sbe"
        ...
        original_file_date = "2024-10-03T05:16:27Z"
        ...
        nomb_proc = "bi_ingestas.raw_interacciones.compra_equipos_sbe"

        // Pipeline Run ID del flujo en Data Factory
        //pipelineRunId = dbutils.widgets.get("pipelineRunId")
        pipelineRunId = "3fa2ae52-9bd9-431f-b167-7a052d525574"
...
println("[INFO - PC] status_ejecucion " + status_ejecucion)
...
df2 = spark.read.option("delimiter", delimitador).option("header", true).
  option("mode", "FAILFAST").csv(path_processing + filename)
display(df2)
//df2.show(false)
