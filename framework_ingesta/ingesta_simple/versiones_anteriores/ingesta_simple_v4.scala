...
spark.sql("CREATE SCHEMA IF NOT EXISTS "+nomb_schema+" MANAGED LOCATION 'abfss://contenedor-test@stbigdatadev02.dfs.core.windows.net/data/producto_asignado' COMMENT 'schema para datos de raw_producto_asignado'")
  spark.sql("CREATE SCHEMA IF NOT EXISTS devtmp.control MANAGED LOCATION 'abfss://contenedor-test@stbigdatadev02.dfs.core.windows.net/data/control' COMMENT 'schema para datos de control'")
} catch {
  case e: Exception =>
    {
      println("[ERROR] " + e)
...
if(!(exists_file(dir_hdfs + "/last_ingest_time.txt"))){ // si no existe el archivo con la fecha maxima, lo crea con la fecha actual (usado por los siguientes archivos al ser procesados)
    println("no existe archivo con fecha maxima  [CREANDO ARCHIVO]");
    makeTxtFile(dir_hdfs + "/last_ingest_time.txt", "timestamp;\n"+string_fecha_modificacion_actual+";")
}else{ // si existe el archivo, lo lee y compara las fechas

  val fecha_modificacion_max = readTxtFile(dir_hdfs + "/last_ingest_time.txt").split("\n")(1).dropRight(1)
  ...
  if(result > 0){
    println("fecha actual mayor que la fecha del archivo  [ACTUALIZAR FECHA DEL ARCHIVO]");
    delete(dir_hdfs + "/last_ingest_time.txt")
    makeTxtFile(dir_hdfs + "/last_ingest_time.txt", "timestamp;\n"+string_fecha_modificacion_actual+";")
  }else{
    println("fecha actual menor o igual que la fecha del archivo [NO HACER NADA]");
  }
}
  ...
  if (list != null && list.length > 0) {
    for (k <- list) {
      ...
      if (!(filename.startsWith("."))) {
        ...
        println("[INFO!!!] FILENAME landing   " + path_landing + filename)
        println("[INFO!!!] FILENAME landing   " + path_processing + filename)
        ...
        println("[INFO] REGISTRA EL DATAFRAME COMO TABLA TEMPORAL")
        ...
        if (subida_datos.toLowerCase().contains("partitioned by")) {
          ...
          if (delete(path_data2)) process_type = "reproceso" else process_type = "normal"
          ...
          val status2 = listFiles(path_data)
          parquetPutName(status2, dataType, plataforma, categoria, loadType, periodicity)
          ...
          final_row_count = spark.read.parquet(path_data2).count
          ...
          println("[INFO] SE CUENTA LA CANTIDAD DE ARCHIVOS GENERADOS")
          ...
          println("[INFO] final_name "+final_name)
          println("[INFO] final_name.indexOf(/+dataType+.) "+final_name.indexOf("/"+dataType+"."))
          println("[INFO] final_name.indexOf(.N) "+final_name.indexOf(".N"))
          
          end_file_name = final_name.substring(0, final_name.indexOf(".N")) + ".snappy.parquet"
          ...
          // spark.sql("VACUUM" + nomb_proc + " SYNC METADATA")
...
control_dataframe.write.mode(SaveMode.Append).saveAsTable("devtmp.control.control_ingestas")
  

// COMMAND ----------

// MAGIC %md
// MAGIC #### Convertir tabla a Delta

// COMMAND ----------

// spark.sql("CONVERT TO DELTA " + nomb_proc)
