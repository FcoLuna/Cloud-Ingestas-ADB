...
try {
  spark.sql("drop SCHEMA if exists devtmp.raw_producto_asignado cascade")
} catch {
  case e: Exception =>
    {
      println("[ERROR] " + e)
...
// PROCESOS CON LAS TABLAS Y ESQUEMAS REVISAAAAAR!!!!!!!!!!!!!
try {
    ...
    for (k <- list) {
      ...
      if (!(filename.startsWith("."))) {
        ...
        if (exists_file(dir_hdfs + "/transformation/transformation.json")) {
          ...
          println("dir_hdfs.split(/).last: " + dir_hdfs.split("/").last)
          dataframe1.createOrReplaceTempView(dir_hdfs.split("/").last + "_view")
        ...
        if (subida_datos.toLowerCase().contains("partitioned by")) {
          ...
          val current = DateTimeFormatter.ofPattern("yyyyMMddHHmmss").format(LocalDateTime.now)
          end_file_name = dataType + "." + plataforma + "." + categoria + "." + loadType + "." + periodicity + "." + current + ".snappy.parquet" // preguntar 
          println("[INFO] final_name "+end_file_name)
