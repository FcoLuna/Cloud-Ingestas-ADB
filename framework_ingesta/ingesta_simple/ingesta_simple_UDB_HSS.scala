...
// MAGIC %md
// MAGIC ### Ingesta Simple manteniendo algunos días de historia
...
val result = Try {
  ...
  val data_source_name = dbutils.widgets.get("filename")
  //val data_source_type = "file"
  //val data_source_name = "HSS_20250421.csv"
...
val config = Try {
  Config(
    ...
    //dir_adls = "abfss://ingestas@stbigdatadev02.dfs.core.windows.net/data/gestion_recursos/inventario_red/calidad_red/udb_hss",
    ...
    //starttime_nifi = "20250423 182802",
    endtime_nifi = dbutils.widgets.get("endtime_nifi"),
    //endtime_nifi = "20250423 183802",
    ...
    //original_file_date = "2025-04-23T11:40:14Z",
    ...
    //nomb_proc = "raw_gestion_recursos.udb_hss",
...
Try {
  println("[INFO] Iniciando procesamiento de archivo")

  val filenameFromWidget = dbutils.widgets.get("filename")
  //val filenameFromWidget = "HSS_20250422.csv"
  val daysToKeep = try {
  dbutils.widgets.get("daysToKeep").toLong
} catch {
  case e: NumberFormatException => 
    throw new IllegalArgumentException("daysToKeep debe ser un número entero")
}
  //val daysToKeep = 5
  if (filenameFromWidget == null || filenameFromWidget.trim.isEmpty) {
    throw new Exception("El valor de 'filename' es nulo o está vacío")
  }
  val cleanedFilename = filenameFromWidget.trim.toLowerCase
  if (!cleanedFilename.endsWith(".csv")) {
    throw new Exception(s"Formato de archivo no soportado: $filenameFromWidget. Se esperaba .csv")
  }
  val datePattern = """(\d{8})""".r
  val bigdataCloseDate = datePattern.findFirstIn(cleanedFilename) match {
    case Some(dateStr) if dateStr.length == 8 =>
      try {
        val parser = new SimpleDateFormat("yyyyMMdd")
        parser.setLenient(false)
        parser.setTimeZone(TimeZone.getTimeZone("Chile/Continental"))
        val parsedDate = parser.parse(dateStr)
        if (parsedDate == null) {
          throw new Exception(s"No se pudo parsear la fecha '$dateStr' como yyyyMMdd")
        }
        format_date.format(parsedDate) // Formato yyyy-MM-dd
      } catch {
        case e: Exception =>
          throw new Exception(s"No se pudo parsear la fecha '$dateStr' del nombre del archivo: $filenameFromWidget. Error: ${e.getMessage}")
      }
    case _ =>
      throw new Exception(s"No se encontró un patrón de fecha válido (yyyyMMdd) en el nombre del archivo: $filenameFromWidget")
  }
  println(s"[DEBUG] bigdataCloseDate extraído: $bigdataCloseDate")

  // Eliminar registros con bigdata_close_date anteriores a días indicados en daysToKeep
  if (listFiles(path_landing).nonEmpty) {
    try {
      // Usar java.time para calcular dateThreshold de manera robusta
      val formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd").withZone(ZoneId.of("Chile/Continental"))
      val closeDate = LocalDate.parse(bigdataCloseDate, formatter)
      val thresholdDate = closeDate.minusDays(daysToKeep)
      val dateThreshold = formatter.format(thresholdDate)
      println(s"[INFO] Eliminando registros con bigdata_close_date <= $dateThreshold")
      spark.sql(s"DELETE FROM delta.`$path_data` WHERE bigdata_close_date <= '$dateThreshold'")
      println(s"[INFO] Registros antiguos eliminados con éxito")
    } catch {
      case e: Exception =>
        println(s"[WARNING] Error al eliminar registros antiguos: ${e.getMessage}")
    }
  }
  ...
  if (stage_files.nonEmpty) {
    ...
    stage_files.filterNot(_.name.startsWith(".")).foreach { file =>
      ...
      var df = spark.read
        .schema(schema_data)
        .csv(s"$path_processing$filename")
      ...
      // Usar bigdata_close_date extraído
      df = df.withColumn("bigdata_close_date", to_date(lit(bigdataCloseDate), "yyyy-MM-dd"))
             .withColumn("bigdata_ctrl_id", lit(bigdata_ctrl_id).cast("long"))
      
      // df.cache()
      
      original_row_count = df.count // Contar después de transformaciones
      
      df.createOrReplaceTempView("ing_schema")
      
      val output_df = spark.sql(select_line)
      
      // Fijar process_type como normal
      process_type = "normal"
      
      // Usar número de particiones optimizado para archivo grande
      val numPartitions = if (config.cant_repartition > 0) {
        config.cant_repartition
      } else {
        50 // Valor recomendado para archivo de ~25GB
      }
      
      // Escribir en modo Append con optimizaciones Delta
      output_df.repartition(numPartitions)
        .write
        .format("delta")
        .mode(SaveMode.Append)
        .option("optimizeWrite", "true") // Optimizar escritura en Delta
        .option("autoCompact", "true") // Compactar archivos pequeños
        .save(path_data)
      
      // Liberar cache si se usó
      // df.unpersist()
