...
val DEBUG: Boolean = sys.env.getOrElse("DEBUG","false").toBoolean
...
if(status_ejecucion == 0){
  try {
    ...
    // add by js: no se puede usar para debugeo esta sección ya que se necesita este valor para operaciones cascada abajo.
    ...
    */
    ...
    // add by js: se quita este copiado temporal
    ...
    // add by js: se quita numPartitions
    df5.write.partitionBy(columns: _*).format("delta").option("partitionOverwriteMode", "dynamic").mode(SaveMode.Overwrite).save(path_data)
