...
// cmd 8
var delimitador, encabezado, dir_adls, pipelineRunId, catalog_control, formato_entrada, dataType, plataforma, categoria, loadType, periodicity, endtime_nifi, original_file_size, formato_salida, dateFormatFile, timestampFormat, dir_adls_2, filename, partition_date_column, last_ingest_time, quote = ""
...
try {
  ...
  timestampFormat = Option(dbutils.widgets.get("timestampFormat")).filterNot(_.isEmpty).getOrElse("yyyy-MM-dd HH:mm:ss.SSSSSSS").replace("|",":")
  ...
  try {
    ...
    if(!(exists_file(dir_adls + "/" + last_ingest_time))){ 
      ...
      if(result >= 0) {
        ...
        delete(dir_adls + "/" + last_ingest_time)
        makeTxtFile(dir_adls + "/" + last_ingest_time, "timestamp;\n"+string_fecha_archivo_procesando+";")
...
if(status_ejecucion == 0){
  try {
    ...
    println("[INFO] select_line: " + select_line)
    ...
    if (exists_file(dir_adls + "/transformation/transformation.json")) {
      ...
      println("[INFO] Entro al else (Sin transformation.json)")

      println(s"[INFO] Leyendo path_landing ${path_landing}/*, delimiter: ${delimitador}, header: ${encabezado}")
      ...
      option("mode", "PERMISSIVE").schema(schema_data).option("DateFormat", dateFormatFile).option("timestampFormat", timestampFormat).csv(path_landing + "/*")
      //PERMISSIVE
      //FAILFAST

      display(df1)
        
      println(s"[INFO] printSchema: ")
      println(df1.printSchema())
      
      val contadorcolumns = df1.count()
      println(s"[INFO] contadorcolumns: ${contadorcolumns}")

      //GUARDA LAS COLUMNAS DEL DATAFRAME
      val groupCols = df1.columns

      println(s"[INFO] llega a guarda las columnas en dataframe")
      // AGRUPA LA DATA CARGADA POR EL NOMBRE DEL ARCHIVO BUSCANDO PARA CADA UNO EL MAXIMO TIMESTAMP
      //CON ESTO SE ASEGURA QUE EN EL REPROCESO SIEMPRE SE TOME EL ARCHIVO REPROCESADO MAS ACTUAL
      val df_ts = df1.groupBy("filename_spark").agg(max("ts").as("ts"))
      println(s"[INFO] df_ts=${df_ts}")

      println(s"[INFO] SE HACE UN JOIN PARA DEJAR SOLO LOS DATASET QUE TENGAN EL TIMESTAMP MAS ACTUAL POR ARCHIVO")
      //SE HACE UN JOIN PARA DEJAR SOLO LOS DATASET QUE TENGAN EL TIMESTAMP MAS ACTUAL POR ARCHIVO
      df_join=df1.join(df_ts,Seq("filename_spark", "ts"))
      println(s"[INFO] df_join.count()=${df_join.count()}")

      println(s"[INFO] SE SELECCIONAN LAS COLUMNAS Y SE ELIMINA LA COLUMNA TS")
      //SE SELECCIONAN LAS COLUMNAS Y SE ELIMINA LA COLUMNA TS
    ...
    println(s"[INFO] original_row_count ${original_row_count}")


    //SE ELIMINAN DUPLICADOS
    println(s"[INFO] SE ELIMINAN DUPLICADOS")
    df3 = df3.dropDuplicates

    //SE CUENTA LA CANTIDAD DE REGISTROS
    val dropDuplicates_row_count = df3.count
    println(s"[INFO] dropDuplicates_row_count ${dropDuplicates_row_count}")


    // ELIMINA FILAS NULAS
    println(s"[INFO] ELIMINA FILAS NULAS")
    df4 = df3.na.drop("all")
    val dropNull_row_count = df4.count
    println(s"[INFO] dropNull_row_count=${dropNull_row_count}")
    ...
    println(s"[INFO] partition_date_column=${partition_date_column}")
    println(s"[INFO] formato_entrada=${formato_entrada}")
    println(s"[INFO] formato_salida=${formato_salida}")
    if (partition_date_column == "") {
      println(s"[INFO] Generando columna [bigdata_close_date] con columna [filename_spark]")
      display(df4.withColumn("test", regexp_replace(element_at(split(col("filename_spark"),"_"), -1), "[^0-9]", "")))
      ...
      println(s"[INFO] Generando columna [bigdata_close_date] con columna [partition_date_column=${partition_date_column}]")
    ...
    */
    println(s"[INFO] Por cada fecha (dates.length=${dates.length})")
    for (i <- 0 until dates.length) {
      path_data2 = path_data
      println(s"\t[INFO] (dates(i)=${dates(i)})")
      if(dates(i) != null){
          for (x <- 0 to splits - 1 by 1) {
            println(s"\t\t[INFO] (x=${x})")
            val parte_fecha = dates(i).toString.split("-")(x)
            val parte_particion = partition_name.split(",")(x).trim()
            println(s"\t\t[INFO] (parte_fecha=${parte_fecha})")
            println(s"\t\t[INFO] (parte_particion=${parte_particion})")
            
            if (x != splits - 1 && splits > 1) {
              path_data2 = path_data2.concat(parte_particion + "=" + parte_fecha + "/")
              columns = columns :+ parte_particion
            ...
            println(s"\t\t[INFO] (path_data2=${path_data2})")
            println(s"\t\t[INFO] (columns.length=${columns.length})")

          }
          path_data2_seq = path_data2_seq :+ path_data2
          counter = counter + 1
          println(s"\t[INFO] (counter=${counter})")
      }
    }
        
    //SE ELIMINAN COLUMNAS Y RUTAS DE PARTICION DUPLICADAS
    path_data2_seq = path_data2_seq.distinct
    println(s"[INFO] (path_data2_seq=${path_data2_seq})")
    ...
    println(s"[INFO] (selectExprs=${selectExprs})")
    ...
    println(s"[INFO] (numPartitions=${numPartitions})")
    
    // SE ELIMINA LO ESCRITO EN LA RUTA TEMPORAL
    println(s"[INFO] SE ELIMINA LO ESCRITO EN LA RUTA TEMPORAL (path_tmp='${path_tmp}')")
    delete(path_tmp)

    //SE ELIMINAN TODOS LOS DIRECTORIOS PARTICIONADOS QUE SE CARGARAN EN CASO QUE EXISTAN
    // deletePartitions(path_data2_seq)
    val df5_count = df5.count()
    println(s"[INFO] (df5_count=${df5_count})")


    // SE ESCRIBE EL RESULTADO CON EL NUMERO DE ARCHIVOS CALCULADOS, SE PARTICIONA POR LA O LAS COLUMNAS DE PARTICION.
    println(s"[INFO] ESCRIBIENDO TABLA (nomb_proc=${nomb_proc})")
    println(s"[INFO] ESCRIBIENDO TABLA (columns=${columns})")
    df5.repartition(numPartitions).write.partitionBy(columns: _*).format("delta").option("partitionOverwriteMode", "dynamic").mode(SaveMode.Overwrite).saveAsTable(nomb_proc)

    // SE ELIMINA LO ESCRITO EN LA RUTA TEMPORAL DE TRANSFORMACIONES
    delete(path_tmp_transform)
      

    // ELIMINAR FOR ( SOLO EXISTE PARA LA TABLA DE CONTROL, NO SE PUEDEN CAMBIAR LOS NOMBRE DE ARCHIVOS DELTA)
