...
// // Variables Pruebas
 ...
 delimitador = ","
 encabezado = "True"
 dir_adls = "abfss://ingestas@stbigdatadev02.dfs.core.windows.net/data/gestion_recursos/operacion_red/ookla/speedtest_data_suplem_sensible"
 pipelineRunId = "53c4ee2e-5c73-4c02-9f26-bedc37d9c489"
 catalog_control = "bidesarrollo.control.control_ingestas"
 ...
 periodicity = "d"
 starttime_nifi = "20241118 125850"
 endtime_nifi = "20241118 010134"
 original_file_size = "2350862"
 original_file_date = "2024-11-22T07:00:38Z"
 formato_salida = "yyyy-MM-dd"
 dateFormatFile = "yyyy-MM-dd"
 timestampFormat = "yyyy-MM-dd HH|mm|ss"
 dir_adls_2 = "abfss://ingestas@stbigdatadev02.dfs.core.windows.net/data/gestion_recursos/operacion_red/ookla/speedtest_data_suplem_sensible/landing/2024/11"
 filename = "SensitiveData_2024-11-21.csv"
 partition_date_column = ""
 quote = "\""
 nomb_proc = "bi_ingestas.raw_gestion_recursos.speedtest_data_suplem_sensible"
 nomb_schema ="bi_ingestas.raw_gestion_recursos"        
...
try {
  ...
  print("split_index_datefile: " + split_index_datefile)
//////////////////////////////////////
...
if(status_ejecucion == 0){
  try {
    ...
    println("datetime_fecha_archivo_procesando: " + datetime_fecha_archivo_procesando)
    string_fecha_archivo_procesando = format_max.format(datetime_fecha_archivo_procesando)
    println("string_fecha_archivo_procesando: " + string_fecha_archivo_procesando)
    ...
    if(!(exists_file(dir_adls + "/last_ingest_time.txt"))){ 
      ...
      println("dir_adls: " + readTxtFile(dir_adls + "/last_ingest_time.txt").split("\n"))

      fecha_ultima_ejecucion = readTxtFile(dir_adls + "/last_ingest_time.txt").split("\n")(1).dropRight(1) 
      println("fecha_ultima_ejecucion: " + fecha_ultima_ejecucion)
      println("datetime_fecha_archivo_procesando: " + datetime_fecha_archivo_procesando)
...
if(status_ejecucion == 0){
  try {
    ...
    println("[INFO] nombre_archivo " + nombre_archivo)
...
if(status_ejecucion == 0){
  try {
    ...
    for (campo <- arreglo_string if campo.trim() != "") {
      ...
      println("[INFO] variable: " + variable)
    ...
    if (exists_file(dir_adls + "/transformation/transformation.json")) {
      ...
      val df_ts = df1.groupBy("filename_spark").agg(max("ts").as("ts"))
      println("df_ts")
      println(df_ts.show(false))

      //SE HACE UN JOIN PARA DEJAR SOLO LOS DATASET QUE TENGAN EL TIMESTAMP MAS ACTUAL POR ARCHIVO
      df_join=df1.join(df_ts,Seq("filename_spark", "ts"))
      println("[INFO] df_join.count: ")
      println(df_join.count.toInt)
// **************************************************
      //SE SELECCIONAN LAS COLUMNAS Y SE ELIMINA LA COLUMNA TS
    ...
    println("[INFO] df3.count: " + df3.count())
    //SE CUENTA LA CANTIDAD DE REGISTROS
    original_row_count = df3.count
    println("[INFO] original_row_count: " +original_row_count)
    ...
    */
    println("Dates.length: " + dates.length)
    ...
    for (i <- 0 until dates.length) {
      ...
      for (x <- 0 to splits - 1 by 1) {
        ...
        println("[INFO] splits: " + splits + " - x:" +x)
        if (x != splits - 1 && splits > 1) {
          println("PASE.....******** path_data2: " + path_data2 + " - dates(i): " + dates(i).toString)
        ...
        println("[INFO] path_data2: **************************")
        println(path_data2)

      }
      path_data2_seq = path_data2_seq :+ path_data2
      counter = counter + 1
      println("[INFO] counter: " + counter)
...
println("[INFO] END: " + end)

//SE FORMATEA EL TIMESTAMP 
var endtime_spark = formatter.format(end)
println("[INFO] endtime_spark: " + endtime_spark)

//SE CALCULA LA DURACION DEL PROCESO SPARK
var totaltime_spark = (end - current).toFloat / 1000
println("[INFO] totaltime_spark: " + totaltime_spark)

println("[INFO] totaltime_nifi: " +totaltime_nifi)

//SE CALCULA LA DURACION DEL PROCESO SPARK + NIFI
var totaltime_process = totaltime_spark + totaltime_nifi.toInt
println("[INFO] totaltime_nifi: " + totaltime_nifi.toInt)

println("[INFO] control_dataframe:")
control_dataframe.show(false)
