...
if(status_ejecucion == 0){
  try {
    ...
    partition_name = subida_datos.substring(subida_datos.toLowerCase().indexOf("partitioned by") + 15, subida_datos.toLowerCase().indexOf("partitioned by") + 15 + subida_datos.substring(subida_datos.toLowerCase().indexOf("partitioned by") + 15).indexOf(")")).replace("(", "").replace("string", "").trim().replace(", hour","")
    ...
    */
    ...
    println(splits)
    // se agrega la variable hour para obtener las horas del archivo
    val hour = filename.split("\\.")(0).takeRight(2)
    for (i <- 0 until dates.length) {
      ...
      // se agregan particiones de hora by JS
      path_data2 = path_data2.concat(s"/hour=${hour}")
      columns = columns :+ "hour"
    ...
    df5 = df4.withColumn("tmp", split($"bigdata_close_date", "-")).select(selectExprs: _*).drop("filename_spark").withColumn("hour", lit(hour))
    println(df5.count)
    println("dfshow")
    df5.show()
