...
var salida, bigdata_close_date, partition_value, process_name, nomb_proc, nomb_schema = ""
  var df1, df2, df3, df_join, df4, df5, df_sal2, df_sal3, df_sal4,df_good,df_bad = null.asInstanceOf[DataFrame]
...
if(status_ejecucion == 0){
  try {
    ...
    if (exists_file(dir_adls + "/transformation/transformation.json")) {
      ...
      option("mode", "PERMISSIVE"). //MODIFICACION 05112024
     ...
     if (df1.columns.contains("_corrupt_record")) {
      println("[INFO] ENTRO AL IF _corrupt_record df1 fila 70")
      df_good  = df1.filter(col("_corrupt_record").isNull)
      df_bad   = df1.filter(col("_corrupt_record").isNotNull)
      df1 = df_good.drop("_corrupt_record","columnNameOfCorruptRecord")
      cantidad_df_bad = df_bad.count()
      println(s"COUNT $cantidad_df_bad")
      ...
      //crea df que identifican los registros corruptos de la lectura del csv MODIFICACION 05112024
     if (df3.columns.contains("_corrupt_record")) {
      println("[INFO] ENTRO AL IF _corrupt_record df3 fila 70")
      df_good  = df3.filter(col("_corrupt_record").isNull)
      df_bad   = df3.filter(col("_corrupt_record").isNotNull)
      df3 = df_good.drop("_corrupt_record","columnNameOfCorruptRecord")
      cantidad_df_bad = df_bad.count()
      println(s"COUNT $cantidad_df_bad")
      ...
      schema(schema_data).
      option("DateFormat", dateFormatFile).
      option("timestampFormat", timestampFormat).csv(path_landing + "/*")
      //PERMISSIVE
      //FAILFAST

     //crea df que identifican los registros corruptos de la lectura del csv MODIFICACION 05112024
     if (df1.columns.contains("_corrupt_record")) {
      println("[INFO] ENTRO AL IF _corrupt_record df1 fila 171")
      df_good  = df1.filter(col("_corrupt_record").isNull)
      df_bad   = df1.filter(col("_corrupt_record").isNotNull)
      df1 = df_good.drop("_corrupt_record","columnNameOfCorruptRecord")
      cantidad_df_bad = df_bad.count()
    ...
    for (i <- 0 until dates.length) {
      ...
      for (x <- 0 to splits - 1 by 1) {
        ...
        if (x != splits - 1 && splits > 1) {
          ...
          println(s"ENTRO AL IF COLUMNS $columns")

        } else if (x == splits - 1 && splits > 1) {
          path_data2 = path_data2.concat(partition_name.split(",")(x).trim() + "=" + dates(i).toString.split("-")(x))
          columns = columns :+ partition_name.split(",")(x).trim()
          println(s"ENTRO AL IF ELSE 1 COLUMNS $columns")
        } else if (x == splits - 1 && splits == 1) {
          path_data2 = path_data2.concat(partition_name.split(",")(x).trim() + "=" + dates(i).toString.split("-")(x))
          columns = columns :+ partition_name.split(",")(x).trim()
          println(s"ENTRO AL IF ELSE 2 COLUMNS $columns")
        }
