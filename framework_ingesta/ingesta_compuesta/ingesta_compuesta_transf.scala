...
if(status_ejecucion == 0){
  try {
    ...
    if (exists_file(dir_adls + "/transformation/transformation.json")) {
      ...
      var tmp_df_join = df_join.select("filename_spark", "ts")
      tmp_df_join.show(false)
      ...
      var test_split = dir_adls.split("/").last
      println("test_split....: " + test_split)
    ...
    */
    ...
    // SE ESCRIBE EL RESULTADO CON EL NUMERO DE ARCHIVOS CALCULADOS, SE PARTICIONA POR LA O LAS COLUMNAS DE PARTICION.
    df5.repartition(numPartitions).write.partitionBy(columns: _*).format("delta").mode(SaveMode.Overwrite).save(path_data)
