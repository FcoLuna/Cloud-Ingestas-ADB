...
val passw_exadata = "f1j0_4md0cs_2021"
...
//PERIODO PARA TRUNCAR TABLA DE DESTINO
val periodo = dbutils.widgets.get("periodo")
//CONDICION PARA EXTRAER DESDE CATALOGO
//val where = dbutils.widgets.get("where_catalogo")
...
/*
println("[INFO] Parametros test fijos=====")
//CATALOGO DEL CUAL SE TOMARÁN LOS DATOS A CARGAR
val catalog = "biproduccion"
//TABLA DEL CATALOGO LA CUAL SE TOMARÁN LOS DATOS A CARGAR
val table_unityCatalog = "producto_asignado.gneta_fijo_suscriptor_producto_cg"
// CONCATENACION DE CATALOGO + SCHEMATABLE
val catalogSchemaTable_ADB = catalog + "." + table_unityCatalog
//COLUMNAS QUE SE RESCATARAN DESDE LA TABLA DEL UNITY CATALOG (PUEDE SER * PARA TODO O SEPARADO POR COMAS - COLUMN_1, COLUMN_2, COLUMN_3)
val columns = "*"
//USUARIO BASE DE DATOS
val user_exadata = "SRV_AMDOCS_FIJO_BIGDATA"
//SECRETO BASE DE DATOS
val passw_exadata = "f1j0_4md0cs_2021"
//val kv_secret_name = dbutils.widgets.get("kv_secret_name")
//ESQUEMA DE LA TABLA A CARGAR
val schema_exadata = "producto_asignado"
//NOMBRE TABLA A CARGAR
val table_exadata = "tbf_gnt_suscriptor_producto_cg"
//ESQUEMA DEL PROCEDIMIENTO ALMACENADO
val schema_sp = "producto_asignado"
//PERIODO PARA TRUNCAR TABLA DE DESTINO
val periodo = "13"
//CONDICION PARA EXTRAER DESDE CATALOGO
//val where = dbutils.widgets.get("where_catalogo")
//CANTIDAD DE PARTICIONES, EN CASO DE NO AGREGAR LO DEJA POR DEFECTO EN 1
val cant_repartition = 1 //Option(dbutils.widgets.get("cant_repartition")).filterNot(_.isEmpty).getOrElse("1").toInt
*/
...
// DBTITLE 1,Base Exadata
 ...
 val passw        = passw_exadata //dbutils.secrets.get("secrets-ingestas",kv_secret_name)
 ...
 val osuser       = user_exadata //"Ingestas"
...
//val query_truncate = s"BEGIN ${schema_sp}.DO_THE_TRUNCATE_PARTITION('"+schemaTable_exadata+"','PARTITION','PART_"+periodo+"'); END;"
...
println("[INFO] QUERY A CONSULTAR: " + "SELECT " + columns + " FROM " + catalogSchemaTable_ADB + " where concat(year,month) >= date_format(trunc(add_months(current_date(),-" + periodo + "),'MM'),'yyyyMM')")

val df = spark.sql("SELECT " + columns + " FROM " + catalogSchemaTable_ADB + " where concat(year,month) >= date_format(trunc(add_months(current_date(),-" + periodo + "),'MM'),'yyyyMM')")
//display(df)
