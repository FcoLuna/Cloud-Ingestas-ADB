...
//CANTIDAD PERIODO PARA TRUNCAR TABLA DE DESTINO
val q_periodos = dbutils.widgets.get("q_periodos_trunca")
...
// Formato de fecha en yyyymmdd
val formatter = DateTimeFormatter.ofPattern("yyyyMMdd")
val cant = q_periodos.toInt
for (i <- 0 to cant) {
  // Restar días a la fecha actual
  val currentDate = LocalDate.now().minusDays(i)
  // Formatear la fecha
  val periodo = currentDate.format(formatter)
  // Arma query con periodo a borrar
  val query_truncate = s"BEGIN ${schema_sp}.DO_THE_TRUNCATE_PARTITION('"+schemaTable_exadata+"','PARTITION','PART_"+periodo+"'); END;"
  println(query_truncate)
  // Ejecuta la query
...
println("[INFO] QUERY A CONSULTAR: " + "SELECT " + columns + " FROM " + catalogSchemaTable_ADB + " " + where)

val df = spark.sql("SELECT " + columns + " FROM " + catalogSchemaTable_ADB + " " + where)
//display(df)
