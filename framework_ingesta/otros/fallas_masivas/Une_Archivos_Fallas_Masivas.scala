...
dbutils.widgets.text("dir_adls_rel","/data/gestion_recursos/operacion_red/fatem/fallas_masivas_finaliz_react_fo")
...
// Leer y combinar los archivos
  ...
  val combinedData = files.flatMap { file =>
    ...
    if ( directoryName.contains("/data/gestion_recursos/operacion_red/fatem/fallas_masivas_nuevas_react_fo") )
      filteredData = data.filter(line => line.startsWith("20") || line.startsWith("INICIO"))
    else if (directoryName.contains("/data/gestion_recursos/operacion_red/fatem/fallas_masivas_finaliz_react_fo"))
    {
      filteredData = data.filter(row => row.matches("^[0-9].*") || row.startsWith("ID_ALARMA"))
      println("filtro 2")
    }
    else
    {
