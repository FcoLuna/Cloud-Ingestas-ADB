...
cant_repartition = int(dbutils.widgets.get("cant_repartition"))
##nuevo
...
if validate:
    ...
    if len(df_inicial.columns) != len(lista_schema) +1:
        ...
        if df_bad.count() > 0:
            df_bad.repartition(cant_repartition).write.format('csv').options(delimiter=delimitador).mode('overwrite').save(f"{ruta_stage}_error/temp", header=encabezado)
...
display(df)
df.repartition(cant_repartition).write.format("csv").options(delimiter=delimitador).mode("overwrite").save(f"{ruta_destino}/temp", header=encabezado)
...
if is_compuesta:
    ...
    if fileExists(ruta_destino, nombre_archivo_landing):
        ...
        df.repartition(cant_repartition).write.format("csv").options(delimiter=delimitador).save(f"{ruta_destino}/temp", header=encabezado)
    else:
