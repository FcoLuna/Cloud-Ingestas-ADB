...
#20241214 Mod by CV(EY): Se agrega option de multiline para csv con celdas con saltos de linea (chattigo)
df = spark.read.option("skipRows", skip_rows).option("multiline","true").option("delimiter", delimitador).option("header", encabezado).option("quote",quote).option("encoding", encoding).csv(f"{ruta_stage}/{nombre_archivo}")
...
if validate:
    ...
    print(lista_schema)
    cantidad_lista_schema = len(lista_schema) +1 #CANTIDAD COLUMNAS ESQUEMA MODIFICACION 20240511
    # VALIDAR NUMERO DE COLUMNAS
    print("cantidad_lista_schema ",cantidad_lista_schema)
    print("cantidad df_inicial.columns ", len(df_inicial.columns))
    if len(df_inicial.columns) != cantidad_lista_schema:
        print(df_inicial.columns)
        print("ERROR CANTIDAD DE COLUMNAS")
        #CANTIDAD COLUMNAS ESQUEMA MODIFICACION 20240511
        df_cantidad_columnas = df_inicial.withColumn("cantidad_columnas", size(split(concat_ws(",", *df_inicial.columns), ",")))
        df_good = df_cantidad_columnas.filter(col("cantidad_columnas") == cantidad_lista_schema)
        df_bad = df_cantidad_columnas.filter(col("cantidad_columnas") != cantidad_lista_schema)
        df = df_good.drop("cantidad_columnas","index_tmp")
...
if is_compuesta:
    ...
    if fileExists(ruta_destino, nombre_archivo_landing):
        ...
        dforiginal = spark.read.option("multiline","true").option("delimiter", delimitador).option("header", encabezado).option("quote",quote).option("encoding", encoding).csv(f"{ruta_destino}/{nombre_archivo_landing}")
