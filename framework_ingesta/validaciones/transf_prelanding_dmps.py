...
if is_dmps:
    if validate_schema_avro:
        # Convertir el esquema Avro a una lista de listas
        lista_schema_avro = ast.literal_eval(schema_avro)

        # Si la longitud del DataFrame no coincide con la del esquema Avro
        if len(df.columns) != len(lista_schema_avro):
            ...
            for columna in lista_schema_avro[len(df.columns):]:
                # columna[0] es el nombre, columna[1] es el tipo
                tipo_columna = eval(columna[1])  # Convertir el string del tipo a clase de PySpark
                # Agregar la columna nueva al DataFrame con el tipo especificado
                df = df.withColumn(columna[0], lit(None).cast(tipo_columna))
...
if is_dmps:
    print("id dmps")
    # Convertir el valor original_file_date al formato yyyyMMdd
    formatted_date = date_format(to_date(lit(original_file_date), "yyyy-MM-dd'T'HH:mm:ss'Z'"), "yyyyMMdd")
    
    # Crear la columna partition_date con el valor formateado
    df = df.withColumn("partition_date", formatted_date)
